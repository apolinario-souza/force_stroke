...
Para investigar uma relação dose-resposta entre exposição ao protocolo e desfechos, calculou-se, por participante e movimento, o total de sessões frequentadas e o valor de Δfmax e ΔRMSE registrado na última sessão disponível, representando a mudança acumulada do início ao fim da participação. A associação entre o número total de sessões e esses deltas finais foi estimada pela correlação de Pearson com IC 95% pela transformação de Fisher. Essa análise é entre participantes e complementa os modelos intraindividuais descritos acima.
...
Para o erro de rastreamento, a flexão de ombro apresentou correlação negativa forte e significativa entre número de sessões e ΔRMSE final (r = −0,80; p = 0,005; IC 95%: [−0,951; −0,343]), indicando que participantes que frequentaram mais sessões apresentaram maior melhora relativa no controle motor desse movimento ao final do protocolo (Tabela 5). Para os demais movimentos, as correlações foram próximas de zero e não significativas.
...
Esse resultado contrasta com a tendência longitudinal de piora do RMSE observada para flexão de ombro nos modelos mistos. Enquanto o grupo como um todo apresentou aumento do erro ao longo das sessões, participantes que acumularam mais sessões alcançaram desfechos finais relativamente melhores, sugerindo que a frequência de participação pode modular o efeito do protocolo sobre o controle motor desse movimento.
